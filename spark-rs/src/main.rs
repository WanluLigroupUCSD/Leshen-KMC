/// spark-rs: Kinetic Monte Carlo & Microkinetic Modeling
///
/// Two modes:
...
///
/// Usage:
///   spark kmc                          # full model KMC
///   spark kmc --cycle --u=-1.0         # cycle-only KMC
///   spark mkm --u=-1.0                 # full mean-field
///   spark mkm --cycle --u=-1.0         # cycle-only mean-field
///   spark scan-u --cycle               # potential scan (cycle mode)
///   spark validate                     # CO/Pd100 Langmuir test
...
#[derive(Parser)]
#[command(name = "spark", about = "Kinetic Monte Carlo & Microkinetic Modeling for N2 Reduction")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
