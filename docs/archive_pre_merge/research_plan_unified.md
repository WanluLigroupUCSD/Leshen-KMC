...
| **SPARK** | **Python + Rust** | **Pairwise NN** | **✓ Butler-Volmer PCET** | **✓ Native** |
...
**SPARK fills this gap** as the first general-purpose framework with native Butler-Volmer rates, polarization curve computation, and integrated KMC + mean-field microkinetic modeling.
...
Validate SPARK against analytical solutions and published data using the simplest electrochemical reaction: HER on Pt(111). This system exercises all framework features (PCET, lateral interactions, diffusion, KMC vs MKM) with only 2 species and 3 reaction steps.

以最简单的电化学反应验证 SPARK：Pt(111) 上 HER。该体系仅含2种物种和3步反应，但能全面检验框架功能。
...
SPARK as the **first open-source general-purpose electrocatalytic KMC framework** enables the broader community to perform spatial KMC for any electrocatalytic system without custom code — lowering the barrier from months of development to hours of model definition.
